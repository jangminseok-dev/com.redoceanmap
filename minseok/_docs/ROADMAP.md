...
| I-7 | ~~**알림 채널 텔레그램 추가**~~ — **완료(2026-08-24)**. `user_alert_settings.telegram_chat_id`(루트 체인 `m2f3a4b5c6d7`) + `PUT /alert-settings` 확장 + 스캔 응답 `telegrams[]` + n8n 워크플로 텔레그램 분기 + 프론트 `/profile` 입력. **운영 잔여: 텔레그램 봇 생성·n8n 자격증명 등록(사용자)**. 승격 재검토: 불발동 — 채널 2개여도 조립은 허브 1 유스케이스, 구독 설정은 토글+chat_id 수준 | Kuma용 봇은 리포 밖 운영 중(리포 코드 0건). 씽크풀·AI시그널은 푸시가 기본. "채널 다변화 시 알림 스포크 승격 재검토" 조건 발동 여부 확인 |
...
다음 순서 권고: I-11~I-18(제약·근거 결정론 가드) → I-4(뜨는 상권 필터 축) → B7(가맹정보 API — 선행 조건 없음) → B3 → lease 게이트. I-2 측정(러너 `-m ollama`)과 I-7 운영 잔여(텔레그램 봇·n8n 자격증명)는 사용자 확인 필요.
