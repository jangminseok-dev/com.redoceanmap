...
| # | 항목 | 선행 조건 | 계획 |
|---|---|---|---|
| E1 | **5분봉 이벤트 연구 확장** (2-3일) | 없음 — 60일치 축적됨 | 뉴스 발행 → +30분/+60분 초과 수익률(1d 연구와 같은 기준선 대비). 장외 발행은 "다음 **존재하는** 5m 봉"으로 매핑(휴장 하드코딩 불요), 5m 소급 60일 한계 명시. `event_study` payload에 short_horizon 섹션 + admin 화면 확장 |
