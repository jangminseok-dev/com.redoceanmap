...
| B8 | **투자금 회수기간 서술** — 브랜드 창업비용(B7) ÷ (상권 점포당 매출 × 마진 가정) → "가정 명시한 회수 N년" 팩트 | 마이프차 양도양수의 "투자금 회수 기간 계산" | `payback`·회수기간 grep 0건. B7 선행. 마진은 결정론 상수 + 가정 명시(I-9 "한계 명시" 원칙과 일관). game `fitness`는 소비만(B4와 동일 판정) | S (B7 후) |
