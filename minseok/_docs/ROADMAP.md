...
| # | 마일스톤 | 선행 | 기간 | 게이트 |
|---|---------|------|------|--------|
| R1 | **검색 품질 계측 기반** — 현행 순수 코사인의 baseline을 결정론 숫자로 고정한다. ① 검색 골든셋 `apps/chat/tests/eval/retrieval_golden.jsonl`(질의 40건 = stock 25 + market 15, **TREC pooling** 라벨 — 코사인 top-20을 사람이 0/1/2 등급) ② 순수 도메인 채점기 `chat/domain/services/retrieval_scorer.py`(`recall@k`·`nDCG@k`·`MRR`·`mAP`, 표준 라이브러리만) ③ 러너 `test_retrieval_runner.py`(`-m ollama`, 실 DB) → `retrieval_trace.jsonl` ④ 게이트 `test_retrieval_gate.py` + `retrieval_baseline.json` — `test_quality_gate.py` 패턴 복제(-3%p 회귀 실패). **⛔ 라벨링을 LLM에 시키지 않는다** — 단일 모델 정책상 7.8B가 자기 검색을 심판하는 순환이 된다(`eval_scorer.py`가 LLM-as-judge를 배제한 것과 같은 이유) | 없음 | 1.5-2일 | baseline 기록 + **같은 트레이스 재채점 시 동일 결과**(결정론) + 계약 5 KEPT |
| R2 | **하이브리드 검색(trigram + 벡터 RRF)** — ① `pg_trgm` 확장 마이그레이션 + `similarity(title, :q)` 키워드 채널 ② RRF 결합(`Σ 1/(60+rank)`)을 stock·market 두 리포지토리의 `search_similar`에 주입 ③ 파라미터 스윕(채널별 top-N·RRF k·가중) ④ **벡터 인덱스 재판정** — 73k 브루트포스 latency 실측 후 p95 미달일 때만 hnsw 도입. **한계 명시**: 형태소 분석기가 없어 문자 trigram이며, 조사·띄어쓰기 변형에는 강하나 의미 확장은 안 된다 — 리포트에 남긴다. **범위 밖**: 리랭커 모델은 `llm_orchestrator`의 3번째 모델이라 **단일 모델 정책 예외 판정이 선행**이다(후보로만) | **R1** | 2-3일 | R1의 **nDCG@5가 baseline 대비 +0.03 이상**. 미달이면 **기각하고 현행 순수 코사인 유지**(E2와 동일 처리) |
| R3 | **청킹 전략(DART 공시 원문)** — 표·각주가 있는 실제 문서에서 청킹 설계가 검색 품질을 바꾸는지 확인한다. **뉴스 본문은 코퍼스로 쓰지 않는다**(Google News RSS는 제목만 주고 `url`이 base64 리다이렉트 — 본문 확보는 스크래핑뿐이고 이는 "비공공 스크래핑 비추천" 원칙 위반). ① DART 공시서류원본파일 API → 워치리스트 한국 종목 10사 × 최신 사업보고서 1건(`dart_corp_codes.json` 캐시 재사용) ② `disclosure_chunks`(stock 체인, 공유 DB) — `corp_code·rcept_no·section_path·chunk_index·kind(text\|table\|footnote)·content·embedding` ③ 청킹 3종 비교: (a) 고정 토큰 512/오버랩 64(대조군) (b) 섹션 헤딩 분할 (c) **표는 행 단위 + 섹션 헤더·표 캡션 승계, 각주는 참조 본문에 인라인** ④ 골든셋에 공시 질의 20건 추가(**표를 봐야만 답하는 질의 최소 8건**). **착수 전 30분 확인**: 원본파일 형식(ZIP 내 XML/HTML)과 표가 HTML `<table>`인지 이미지인지 — **이미지면 R3는 여기서 중단**하고 그 사실을 리포트에 남긴다 | **R1** (R2와 병행 가능) | 3-4일 | (c)가 (a) 대비 **표 질의 `recall@5` +0.10 이상**. 미달이면 청킹 고도화 기각, (a) 고정 토큰 유지 |
| R4 | **문장 단위 출처 인용** — 현행은 근거 카드를 답변 **옆에** 붙일 뿐 어느 문장이 어느 문서에서 나왔는지 매핑이 없다(프롬프트가 "기사 제목 인용 가능"이라고 말하는 수준). ① 컨텍스트 블록의 근거에 번호를 부여하고 답변 문장 말미에 `[1][2]` 마커를 요구하도록 `PHASE2_PROMPT`·`stock_answer`·`market_news_answer` 고도화 ② `eval_scorer.py`에 `citation_coverage`(수치·사실 주장 문장 중 마커가 붙은 비율)와 `dangling_citation`(존재하지 않는 번호 인용 = 새 환각 유형) 추가 ③ 프론트에서 마커를 근거 카드로 앵커 연결. 기존 하네스에 지표 2개를 얹는 구조라 신규 테이블·신규 모델이 없다 | **R1** 불요 (독립) | 0.5-1일 | `citation_coverage`를 baseline에 등록 + `dangling_citation` **0건을 절대 규칙**으로 승격(환각 숫자와 달리 결정론 검증이 가능하다) |
...
| M7 | **개인화 ① — 관심 목록 상태 보드** (recommendation 확장). ③-M2가 만든 `/bookmarks`는 "찜한 것으로 돌아가는 재개 지점"(라벨+등록일+삭제)일 뿐 **상태가 없다**. 이걸 "내 관심 대상의 지금"으로 승격한다: 종목은 방향 판정·등락률·최신 신호, 상권은 종합점수·전분기 대비. 데이터·판정 로직은 전부 기존 것(허브 `StockForecastPort`·`StockAnalysisPort`·`CommercialDataPort`)이라 **새 수집·새 모델·새 테이블이 없다** — 조합만 한다. **⛔ 홈(`app/(seoul)/page.tsx`)에는 넣지 않는다** — "물어보는 자리 하나만 둔다, 설명 문구·퀵 칩·카드 목록을 두지 않는다"는 2026-08-16 리뉴얼 결정을 유지한다. 개인화의 자리는 `/bookmarks`다 | 2-3일 | **게이트는 효과가 아니라 격리다** — 사용자 A의 토큰으로 B의 북마크·상태가 절대 보이지 않음을 회귀 테스트로 고정(`test_admin_access_matrix.py` 선례). 북마크 0건·조회 실패 종목은 열화 동작(빈 상태 문구, 보드 자체는 뜬다) |
