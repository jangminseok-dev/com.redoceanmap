...
## 오케스트레이터 역할 확정 + 임베딩 교체 (2026-09-15 오후)

**문제**: "오케스트레이터"는 이름뿐이었다 — 레지스트리(`register`/`ModelSpec`)는 있지만 모델 하나만 등록됐고,
Gemini는 허브 어댑터가 직접 REST를 호출해 "모든 LLM 호출이 수렴한다"는 문서 문장이 거짓이었다.
임베딩 bge-m3는 BAAI(중국) 모델이라 "중국 모델 배제" 제약과도 어긋났다.

**결정(사용자, 2026-09-15)**: 이름을 유지하고 역할을 채운다 — 세 갈래를 실제로 분기하는 오케스트레이터로.
1. 로컬 생성(`LLM_MODEL`) — 사고 모드는 env 기본 + 호출 단위 `think=` 덮어쓰기(배치 판단용 레버, 아직 켠 곳 없음).
2. 임베딩(`EMBED_MODEL`) — 질의/문서 프롬프트를 모델별로 붙인다(`kind="query"|"document"`, 포트 3종에 전파).
3. 외부 생성(Gemini) — `core/llm/gemini_client.py`로 이동, 허브 `GeminiApiAdapter`는 위임만.
   `LLM_FALLBACK=external`이면 Ollama 장애(접속 불가·5xx)에서 같은 프롬프트(system·history·JSON 모드 포함)를
   Gemini로 보낸다. 4xx(프롬프트·모델 문제)는 폴백하지 않는다. 평가 러너는 off.
레지스트리는 제거(사용처: 골든셋 러너의 무해한 한 줄뿐). 단일 모델 정책은 "생성 하나 + 임베딩 하나 + 외부 폴백"으로 개정.
회귀 테스트 `minseok/tests/test_llm_context_window.py`(사고 모드·프롬프트·폴백 6건 추가).

**임베딩 후보 비교** — 골든셋의 라벨된 후보 풀만 각 모델로 임베딩해 코사인 재순위를 같은 채점기로 채점
(전체 코퍼스 재임베딩 없이 상대 비교. 뉴스 40문항·공시 20문항, 스크립트는 세션 스크래치패드).

| 모델 | 출처·라이선스 | 차원 | 뉴스 nDCG@5 / R@10 / MRR / mAP | 공시 nDCG@5 / R@10 / MRR / mAP | ms/문서(제목·청크) |
| --- | --- | --- | --- | --- | --- |
| bge-m3(현행) | BAAI 중국, MIT | 1024 | 0.762 / 0.404 / 0.958 / 0.875 | 0.329 / 0.176 / 0.486 / 0.397 | 34 · 29 |
| multilingual-e5-large | Microsoft, MIT | 1024 | 0.828 / 0.439 / 1.000 / 0.935 | **0.408 / 0.288 / 0.632 / 0.470** | 83 · 118 |
| **embeddinggemma** | Google, Gemma Terms | 768 | **0.902 / 0.461 / 1.000 / 0.960** | 0.359 / 0.214 / 0.659 / 0.437 | 36 · 26 |
| granite-embedding 278m | IBM, Apache 2.0 | 768 | 0.760 / 0.412 / 0.975 / 0.893 | 0.295 / 0.239 / 0.469 / 0.395 | 34 · 19 |

**확정: embeddinggemma** — 주 소비처(뉴스 RAG)에서 가장 좋고 공시도 현행보다 낫다. e5는 공시에서 앞서지만 2~3배 느리다.
프롬프트("task: search result | query: " / "title: none | text: ")를 붙인 값이며 오케스트레이터가 같은 프롬프트를 쓴다.
컨텍스트 2,048토큰 — 공시 청크 최대 1,994자·p95 512자라 안에 든다. 라이선스는 Gemma Terms(E5가 상용 가능으로 분류).

**적용**: ORM 4종 `EMBEDDING_DIM=768`, 마이그레이션 공유 DB `q6d7e8f9a0b1`(news_articles·disclosure_chunks·
inbound_mails·market_news_articles) + market DB `e9a1b2c3d4e5` — 기존 벡터는 USING NULL로 비우고 재임베딩:
뉴스 106,547건(프로덕션 `embed_pending` 루프, 최신 우선) + 공시 청크 101,060건(`collect_disclosures.py --embed`)
+ 상권뉴스 3,181건. 재임베딩 동안 벡터 검색은 채워진 범위만 돈다(하이브리드의 trigram 채널은 그대로).
완료 뒤 검색 러너 → `retrieval_gate` → baseline 재박제(bge-m3 baseline은 비교 대상이 아니다 — 모델 교체는 baseline 교체 사유).
Ollama에서 bge-m3·후보 2종은 제거.

**실코퍼스 검색 러너 결과와 풀링 편향(2026-09-15 저녁)** — 재임베딩 뒤 러너(40문항)를 돌리니 코사인 nDCG@5 0.443·
MRR 0.674로 bge-m3 baseline(0.765·0.963)보다 훨씬 낮게 나왔다. 원인은 **평가 라벨의 풀링 편향**이다: 골든셋 라벨은
bge-m3·하이브리드가 찾은 후보(TREC pooling)에만 붙어 있어, 새 모델이 찾아낸 문서는 라벨이 없고 0점 처리된다.
- 상위 10 중 풀 밖 문서 234/400(58%). RS01 "삼성전자 반도체 실적" 상위 8건 중 미라벨 6건은 "[속보] 삼성전자 2분기
  반도체 영업이익 89.5조" 류 — 사람이 보면 정답급.
- DB에 저장된 벡터로 **라벨된 풀만** 재순위하면 stock 25문항 nDCG@5 0.873·MRR 1.0 — 저장 벡터·프롬프트 경로는 정상.
- 하이브리드(벡터+trigram)는 같은 편향 속에서도 nDCG@5 0.620(코사인 0.443 대비 +0.178).
조치: `retrieval_trace*.jsonl`은 HEAD(bge-m3 스냅샷)로 되돌려 기본 스위트 게이트를 깨지 않았고, 새 러너 트레이스는
세션 스크래치패드에만 있다. **baseline은 재박제하지 않았다** — 편향된 값으로 기준을 낮추면 안 된다. 새 풀의 미라벨
후보 707건(40문항)을 `_docs/qa/RETRIEVAL_RELABEL_2026-09-15.md` 시트로 뽑았다. 사람이 라벨을 채운 뒤 러너 → 게이트 →
baseline 재박제 순서. 그 전까지 검색 게이트는 "bge-m3 스냅샷 기준"이며 새 모델의 실코퍼스 수치는 미확정이다.

**재라벨과 확정(2026-09-15 밤)** — 사용자가 LLM 라벨링 예외를 허용해 Claude가 새 풀의 미라벨 707건을 기존 사람 라벨의
2·1·0 기준에 맞춰 판정했다(시트 `_docs/qa/RETRIEVAL_RELABEL_2026-09-15.md`에 등급·근거 기록 — 검색 모델과 무관한 외부
모델이라 "7.8B가 자기 검색을 심판"하는 순환은 아니다). 같은 라벨(총 1,000여 건)로 네 트레이스를 다시 채점:

| 트레이스 | nDCG@5 | nDCG@10 | R@5 | R@10 | MRR | mAP | market / stock nDCG@5 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| bge-m3 코사인(구 프로덕션) | 0.743 | 0.712 | 0.124 | 0.234 | 0.963 | 0.392 | 0.841 / 0.684 |
| bge-m3 하이브리드 | 0.762 | 0.705 | 0.128 | 0.238 | 0.969 | 0.397 | 0.801 / 0.739 |
| **embeddinggemma 코사인(현 프로덕션)** | **0.878** | **0.835** | **0.141** | **0.264** | **1.000** | **0.469** | 0.893 / 0.869 |
| embeddinggemma 하이브리드 | 0.823 | 0.773 | 0.134 | 0.255 | 1.000 | 0.443 | 0.839 / 0.813 |

- 라벨이 늘어 recall·mAP의 분모가 커졌으므로 **구 baseline(bge-m3, 옛 라벨)과의 recall 비교는 무효**다 — 위 표처럼 같은 라벨로
  비교해야 한다. 그 기준으로 embeddinggemma 코사인이 전 지표에서 앞선다.
- 하이브리드(벡터+trigram RRF)는 새 모델에서는 코사인보다 낮다(−0.055). 프로덕션은 `search_similar`(순수 코사인)라 그대로 두고,
  R2 하이브리드 채택 판단은 "임베딩 모델이 약할 때만 이득"으로 정정한다.
- `retrieval_baseline.json`을 embeddinggemma 코사인 값으로 재박제, `retrieval_trace*.jsonl`은 새 러너 트레이스로 교체.
  eval 게이트 전체 통과.
