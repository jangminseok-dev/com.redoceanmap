...
def _to_schema(item: BookmarkBoardItem) -> BookmarkBoardItemResponse:
    ...
    if item.area is not None:
        # "전분기 대비"는 점수 추이의 최신 매출 QoQ에서 뽑는다 — 점수 v2(2026-09-17)가 매출 성장을 컴포넌트에서
        # 뺐기 때문(향후 폐업률 예측력 0). 서울 비교값은 v2에 없어 None(프론트가 숨긴다)
        latest = next((t for t in reversed(item.area.trend) if t.sales_qoq is not None), None)
        area = BoardAreaStatus(
            total=item.area.total, grade=item.area.grade,
            sales_qoq_pct=latest.sales_qoq if latest else None,
            seoul_qoq_pct=None,
        )
...
async def introduce_myself() -> BookmarkBoardMyselfResponse:
    return BookmarkBoardMyselfResponse(
        ...
        description=(
            ...
            "서울 중앙 상권 대비 종합점수와 전분기 대비 매출 변화를 붙입니다. 상태를 만들 데이터가 "
