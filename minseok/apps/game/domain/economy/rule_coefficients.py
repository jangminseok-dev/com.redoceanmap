...
# 방향만 맞고 크기는 가정이다.
#
# ⚠️ 0.21은 현실 임대료율(매출의 10~15%)보다 높다. 게임에서 **이 값이 난이도를 만든다** —
...
#
# 0.18 → 0.21 (2026-08-03): 0.18에서 실측 중앙값 +22.9%·손실 43.8%로 §3-4 목표를 벗어났다.
# 격자 탐색 실측상 **이 계수가 손실 비율을 거의 단독으로 정한다**(임대료가 실매출이 아니라
# 상권 평균 매출에 고정되므로). 0.21에서 중앙값 -1.4%·손실 51.1%로 목표 밴드에 든다.
RENT_RATIO = Coefficient(0.21, source="rule")  # 월임대료 ÷ 점포당 월매출
RENT_LOCATION_MIN = Coefficient(0.6, source="rule")  # 유동인구 하위 상권
RENT_LOCATION_MAX = Coefficient(1.6, source="rule")  # 유동인구 상위 상권
DEPOSIT_MONTHS = Coefficient(4.0, source="rule")  # ⚠️ 실제 관행은 10개월 안팎이다.
# 게임은 4개월로 낮춘다 — 10개월이면 초기 자본으로 어떤 규모도 창업할 수 없다.
# 2.0 → 4.0 (2026-08-03): 손익의 **배율만** 바꾸는 계수다(실측: 손실 비율이 43.8%에서
# 움직이지 않는다 — 모든 항이 store_scale에 비례해 손익 부호가 규모와 무관하기 때문).
# 분포 폭을 조여 p10 -123.6% 같은 극단을 줄이면서 실제 관행과의 괴리도 절반으로 좁힌다.
...
# --- 동시 운영 가게 수 ---------------------------------------------------------
# n+1호점은 **흑자 분기 결산 n회**가 쌓여야 연다(1호점은 무조건). 판정에 필요한 자료가
# 분기 결산 조회 하나로 끝나고, "한 곳을 굴려본 뒤에 늘린다"는 순서를 만든다.
# 상한 3은 재계산 비용(시즌 말 가게 3개 ≈ 11ms)과 목록 화면이 감당하는 크기에서 왔다.
...
#
# 0.02 → 0.0005 (2026-08-03): **이 게임은 넣은 자본에 비례해 벌고 잃는다.** 하한이 0.02면
# 점포당 월매출이 큰 상권은 초기 자본 90만원으로 아예 못 들어갔다(실측: 전체 조합의 38.9%가
# 창업 거절). 하한을 낮추면 그 상권도 "아주 작게" 들어갈 수 있고, 매출·비용·손익이 모두
# store_scale에 비례하므로(simulate_day의 모든 항) 규모만 작아질 뿐 게임은 그대로 성립한다.
# 실측: 하한 0.005 → 88.0% · 0.002 → 96.3% · 0.001 → 98.7% · **0.0005 → 99.7%** 창업 가능.
# 하한을 0으로 두지 않는 이유는 손님 0명짜리 가게가 만들어지지 않게 바닥을 두기 위해서다.
MIN_STORE_SCALE = Coefficient(0.0005, source="rule")
