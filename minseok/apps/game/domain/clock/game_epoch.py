...
# 시즌 1 재시작 — 2026-08-03 13:00 KST. 확정 후 불변이다(20단계 배포, game-strategy §15).
#
# 앞선 값(2026-07-31 17:00 KST)을 그대로 두면 재시작한 시즌이 이미 66일차부터 시작한다 —
# 아무도 플레이하지 않은 과거가 지나간 상태가 된다(현실 1시간 = 게임 1일). 그래서 재시작
# 배포 시각으로 옮겼다. 빌드·재기동에 시간이 걸리므로 과거가 아니라 가까운 미래 정시로 잡는다.
# 이 시각 전까지는 `tick_at`이 0을 돌려주므로 게임은 0틱에 머문다.
#
# 데이터를 전부 비우고 여는 재시작이라 소급될 과거가 없다 — `GAME_EPOCH_ID`는 1로 둔다.
GAME_EPOCH_START_UTC = datetime(2026, 8, 3, 4, 0, 0, tzinfo=timezone.utc)
