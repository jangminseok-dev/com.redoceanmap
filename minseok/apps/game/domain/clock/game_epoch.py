...
# 시즌 1 시작 — 2026-07-31 17:00 KST. 확정 후 불변이다.
#
# 원래 09:00 KST로 박혀 있었는데 그대로 배포하면 게임이 7일차(현실 1시간 = 게임 1일)부터
# 시작한다 — 아무도 플레이하지 않은 과거가 이미 지나간 상태가 된다. 배포 시각으로 옮겼다.
#
# ⚠️ **재시작 대기(20단계).** 11~19단계에서 규칙이 바뀌어 시즌을 다시 연다. 배포할 때
# 이 값을 **그때의 실제 시각**으로 옮기고 `game_*` 테이블을 비운다(game-strategy §15).
# 여기를 안 고치면 새 시즌이 이미 수백 일차부터 시작한 상태가 된다.
