...
| 항목 | 실측 | 확인 방법 |
|---|---|---|
| game 앱 코드 | **3단계 완료.** 슬라이스 4개(`rulebook`·`market_price`·`wallet`·`trade`) + 도메인 5모듈. 테스트 91개 통과 | `PYTHONPATH=apps python3 -m pytest apps/game -q` |
...
| 프론트 | `/game` 시세 화면(2단계). 매매 UI는 4단계 | `www/app/(seoul)/game/` |
