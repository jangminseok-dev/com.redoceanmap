...
| `GAME_EPOCH_START_UTC` | `2026-08-03 13:00 KST`(= 04:00 UTC), 이후 불변 | 시즌 시작 시각. 20단계 재시작 배포 시각 |
...
| ~~레버리지·강제청산~~ | **18단계에서 도입(판정 번복, 2026-08-03).** 만료(180틱)가 청산 스캔 범위를 묶어 O(log) 설계를 지킨다 — 실측 8ms/포지션·동시 5개 상한이라 지갑 조회 최악이 43ms다. 미접속 중 마감분은 복귀 시 결과로 보여준다. 1배는 도입 전과 동일 |
| 랭킹·리더보드·PvP·길드 | 공용 시장이라 자연스러워 보이지만 조작 유인과 감사 부담이 붙는다. 수요 확인 후 재검토 |
| ~~커뮤니티(UGC)~~ | **2026-08-04 도입(판정 번복, 사용자 수요 확인).** 위 항목의 "수요 확인 후 재검토" 조건이 충족됐다. 감사 부담은 범위로 갚는다 — 신고는 접수만(자동 숨김 없음), 숨김은 어드민이 사유와 함께. **랭킹으로 번지지 않게** 수익률·공감·정렬 축을 만들지 않고 '보유 중' 여부만 붙인다. 작성자는 실명이 아니라 user_id 유도 가명이라 auth 변경·신규 허브 포트가 없다 |
...
- ~~**`GAME_EPOCH_START_UTC` 실값.**~~ **확정: 2026-08-03 13:00 KST**(20단계 재시작 배포 시각).
  그 뒤 불변이다 — game-strategy §15.
