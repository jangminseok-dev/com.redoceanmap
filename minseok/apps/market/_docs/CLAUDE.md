...
- **2026-09-18 가중치 재적합**: 교정된 데이터로 3축 가중치를 격자 탐색(0.05 간격, 학습 2021~2023 / 검증 2024~, 분기 내 Spearman 평균).
  현행 .45/.33/.22는 학습·검증 0.369였고, 학습 최적 **.55/.35/.10**이 검증 0.388로 유지됐다(주변 조합 0.386~0.391로 평평 — 과적합 신호 없음).
  적용 후 백테스트: 우수 1.85% → 양호 2.23% → 보통 2.69% → 주의 3.08% → 위험 3.47%(단조, 상·하위 격차 1.50 → 1.62%p).
  같은 입력의 총점이 낮아지므로(매출 축 비중 ↓) 등급 경계(80/65/45/30)는 그대로 두고 분포만 이동한다.
- **2026-09-18 상권명 복원**: 내려받은 CSV에 가운뎃점이 **실제 물음표(0x3F)로** 들어 있었다(인코딩 문제가 아니다).
  열린데이터 API 원본은 정상(종로·청계 관광특구 · 종로1·2·3·4가동) — `ingest_seoul_3nf.clean_name`이 한글/숫자 사이 '?'만
  가운뎃점으로 되돌리고, 적재된 8건(상권 1 · 행정동 7)은 같은 규칙으로 UPDATE했다. 회귀 테스트 `tests/test_ingest_seoul_names.py`.
