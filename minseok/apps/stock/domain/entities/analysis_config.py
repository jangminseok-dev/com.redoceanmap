...
class AnalysisConfig:
    ...
    def default(cls) -> "AnalysisConfig":
        """(2026-09-21부터 analyze 경로에서 쓰지 않는다 — 분석도 예측과 같은 활성 검증 조합을 DB에서 읽는다. 감성 서프라이즈를
        0.2로 얹은 이 조합은 재검증에서 감성이 무신호였고, 같은 종목의 방향이 분석 화면과 예측에서 갈리는 원인이었다.
        백테스터의 기본 인자와 테스트가 참조해 남겨 둔다.)

        뉴스 감성 비중을 0.5에서 0.2로 낮춘 조합(2026-08-28).
