...
## 일봉 완성 규칙 — 수집 시각 수정 + 분석의 최신 마감 봉 보충 — 2026-09-21

분석 입력을 수집 일봉으로 통일하자 **마감 봉이 분석에 늦게 반영되는** 부작용이 드러났다. 수집 스크립트가 일봉을 "시작 + 24시간" 뒤에야 완성으로 봐서
한국 봉은 마감(15:30) 13시간 반 뒤인 다음 날 00시, 미국 봉은 마감(05:00 KST) 8시간 뒤인 13시에야 DB에 들어왔다. 평일 16:10의 "마감 봉 반영" cron은
이 규칙 때문에 그 시각에 한 번도 마감 봉을 담지 못했다.

- **완성 규칙 단일 정의처**: `domain/services/bar_completeness.py`(순수·표준 라이브러리만) — 일봉은 **시작 + 17시간**이면 완성(미국 16:00·한국 15:30·
  수능일 16:30·VIX 15:15 현지 마감이 전부 지난 뒤 + 약 1시간 여유. ts 기준 상대 시간이라 서머타임 무관). 24시간 거래 상품(`-USD`·`-KRW`·`=X`·`=F`)은 24시간.
  여유를 두는 이유: 수집은 DO NOTHING upsert라 한번 담은 봉을 고치지 않는다 — 마감 직후의 미확정 값을 담지 않는다.
- **수집**(`scripts/collect_prices.py`): 일봉만 이 규칙을 쓴다(5분봉은 그대로 봉 길이). 매시 5분 cron이 한국 봉을 17:05, 미국 봉을 06:05/07:05(서머타임/표준시) KST에 담는다.
  스크립트는 호스트 cron이 저장소 파일을 직접 실행하므로 **배포 없이 저장 즉시 적용**된다. 루트 `venv/`(경량)에서 import되도록 규칙 모듈에 외부 의존을 넣지 않는다.
- **분석**(`StockInteractor._with_newer_closed_bars`): 수집 일봉 뒤에, 아직 수집되지 않은 **마감된** 새 봉을 벤더 이력에서 덧붙인다 — 수집이 늦거나 한두 시간 멈춰도
  분석은 최신 마감 봉을 본다. 벤더 이력은 현재가 조회 때문에 같은 요청에서 이미 받아 둔 것(요청 단위 어댑터의 `_history_cache`)이라 **추가 호출이 없다**.
  진행 중인 봉은 붙이지 않는다(검증 조건 = 마감 일봉. 장중 봉이 끼면 분석·예측 점수가 다시 갈린다). 보충 조회 실패는 수집분만으로 계산(열화).
- **가격 기준 가드**(`SCALE_MISMATCH` 0.10): 수집분 마지막 봉과 같은 날의 벤더 종가가 10% 넘게 다르면(액면분할 등 — 수집에는 분할 처리가 없다) 이어 붙이지 않고
  그 요청은 벤더 이력으로 계산한다 + 경고 로그. 배당 소급 조정의 실측 괴리는 최대 2.9%(83종목)라 걸리지 않는다. **수집분 자체의 분할 미처리(예측·스냅샷 쪽)는 남은 위험이다.**
- 실측(9/21 21:57 KST): 옛 규칙은 삼성전자 9/21 봉을 자정까지 못 담고 새 규칙은 담는다. 분석 보충은 삼성전자 +1봉(수집분은 9/18까지), AAPL·KO 0봉, 같은 날 종가 수집=벤더 일치.
- 남은 차이: 예측 스냅샷은 14:00에 동결되므로, 한국 종목은 17시~다음 날 14시 사이 분석이 최신 스냅샷보다 한 봉 앞설 수 있다(실시간 `forecast` 조회는 DB 봉을 읽어 분석과 같다).
