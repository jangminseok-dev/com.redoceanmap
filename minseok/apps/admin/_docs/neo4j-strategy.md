...
**정기 실행 — ✅ cron 등록 완료 (2026-07-30)**

```
15 2 * * * docker exec redoceanmap-backend-1 python scripts/project_graph.py >> ~/project_graph.log 2>&1
```

- **호스트 `venv`가 아니라 컨테이너 경유다.** 다른 cron(뉴스·시세·라벨)은 허브 HTTP를 부르지만
  이 스크립트는 market DB와 Bolt에 직접 붙는다 — 드라이버(`neo4j` 6.2.0)와 `NEO4J_*`가
  **이미지·`env_file`에 이미 있어** 호스트 venv에 의존성을 더할 이유가 없다
  (호스트 `venv`에는 `neo4j` 미설치 — 확인함).
- 시각은 **상권 뉴스 수집(01:30) 뒤 · 뉴스 라벨링(02:30) 앞**. `Article`·`Topic` 투영이
  그날 수집분을 반영하도록 순서를 잡았다.
- 최초 실행 시 있었던 환경변수 주입 우회(`docker exec -e NEO4J_URI=...`)는 **더 필요 없다** —
  이미지 재빌드로 `neo4j-graphrag`가 구워지고 `.env`의 `NEO4J_*`가 정상 주입됨을 확인했다(§8).
- 실패해도 서비스에 영향 없다(그래프는 파생본, 앱 런타임 질의 코드 0건). 로그만 남긴다.
...
- ~~`neo4j-graphrag`가 이미지에 안 구워져 있다~~ → **해소**(2026-07-30 이미지 재빌드).
  `neo4j-graphrag 1.18.0` · 드라이버 `6.2.0`이 이미지에 있고 `NEO4J_*`도 `env_file`로 주입된다.
  투영 스크립트 2개(`project_graph.py`·`graph_constraints.cypher`)도 이미지에 포함돼
  컨테이너 재생성에도 살아남는다.
