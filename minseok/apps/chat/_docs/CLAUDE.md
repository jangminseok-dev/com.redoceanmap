...
### 종목 비교가 상권 비교로 빠짐 · 같은 대화에서 결론이 뒤집힘 (2026-09-21 실대화 344·340)

- **라우팅 가드**: 비교 게이트는 의도 분류보다 앞에 있고, '비교' + 이전 대화면 질문의 대상이 지역인지 보지 않고 직전 추천 카드를 비교했다 —
  햄버거집 질문 뒤 "샌디스크랑 테슬라 비교해줘"에 직전 상권 3곳이 나왔다. 질문이 비교 대상을 **둘 이상 직접 말했는데**(`_named_compare_operands` —
  지시어·순번·비교 어휘·불용어 제외) 그중 지역이 하나도 없으면(`_mentioned_codes`) 상권 비교를 건너뛰고 phase0로 넘긴다.
  "둘이 비교해줘"·"1번이랑 2번"처럼 대상을 생략·지칭한 질문은 그대로 직전 카드 비교다. chat에는 결정론 종목 해석기가 없어 "종목이면"이 아니라 "지역이 아니면"으로 거른다.
- **종목 비교는 우열을 말하지 않는다**(`stock_verdict` — 축별 다수결·★ 폐지). 3:3을 방향 점수 0.05 차이로 "테슬라 우위"라 했다가 1분 뒤
  "샌디스크 우위"로 뒤집혔고, 사용자는 그 결론을 추천으로 읽었다(권유 금지 원칙). 채점 기록(4,104건·기준일 73일·80종목)에서 같은 날 두 종목을 짝지어
  값이 높은 쪽이 이후 수익률도 높았던 비율: 방향 종합 점수 50.4%(5일)·51.9%(20일), 12-1 모멘텀 50.5%·48.2% — 동전 던지기. 뉴스 감성은 8/20 이벤트 연구에서
  역방향(강한 부정 뒤 초과수익 +2.3%, 강한 긍정 뒤 -0.46%), OBV·거래량·펀더멘털 개수는 검증할 기록이 없다. **갈린 축은 변동성뿐**: ATR이 큰 쪽이
  이후 낙폭도 더 깊었던 비율 61.8%·58.0%. 그래서 결론 줄은 "우열은 말하지 않아요 + 흔들림 차이"만 방향성 있게 말하고, 근거·권유 아님은 보조 문단,
  나머지는 **기준별로 보면**(덜 흔들리는 쪽·최근 1년 더 오른 쪽·60일 범위에서 더 낮은 위치·재무 경고가 적은 쪽 — 고르는 건 사용자. 감성은 역방향 근거라
  기준에서 빼고 표에만). 한계: 약 3.5개월·미국 대형주 위주·겹치는 표본, 모멘텀은 원래 몇 달 단위 요인 — "우리 데이터에서는 못 갈랐다"까지다.
  축 동률은 **화면에 보이는 값**으로 가린다(`judge_axes` — "0.9배 vs 0.9배"는 동률, 상권 축에도 적용). 상권 비교의 1순위(검증된 등급 기준)는 그대로다.
- **값 스냅샷**: 종목 비교 답은 `compareSet.snapshot`(질의별 `StockCompareItem` 전체 + 못 쓴 데이터)을 payload에 남기고, 같은 대화의 후속은
  30분(`_COMPARE_SNAPSHOT_TTL`) 안이면 그 값을 그대로 쓴다 — "그래서 뭐가 나아"에 다시 분석하면 시세·뉴스가 조금씩 달라 1분 사이 결론이 뒤집혔다.
  새 종목을 더하면 그 종목만 분석한다. 필드가 바뀐 옛 스냅샷은 버린다.
- 뒤집힌 직접 원인은 stock 쪽 뉴스 감성이었다(→ stock CLAUDE "현재 감성은 저장 라벨").
